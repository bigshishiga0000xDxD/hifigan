...
class LJSpeechDataset(BaseDataset):
    def __init__(self, root: str, part: str, train_split: float | None = None, *args, **kwargs):
        self.root = Path(root)
    ...
    def _random_split(
        ...
        return np.take(index, perm[:train_size]).tolist(), np.take(index, perm[train_size:]).tolist()
